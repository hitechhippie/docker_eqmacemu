...
if [ -d /var/lib/mysql/${DB_NAME_LS} ]; then
  ...
  mariadb --user root --password=${MARIADB_ROOT_PASSWORD} -h db -D ${DB_NAME_GAME} < /data/alkabor_2023-10-11-15\:19.sql
  echo "[database] load player tables..."
  mariadb --user root --password=${MARIADB_ROOT_PASSWORD} -h db -D ${DB_NAME_GAME} < /data/player_tables_2023-10-11-15\:19.sql
  echo "[database] load data tables..."
  mariadb --user root --password=${MARIADB_ROOT_PASSWORD} -h db -D ${DB_NAME_GAME} < /data/data_tables_2023-10-11-15\:19.sql
  echo "[database] load loginserver tables..."
  mariadb --user root --password=${MARIADB_ROOT_PASSWORD} -h db -D ${DB_NAME_LS} < /data/login_tables_2023-10-11-15\:19.sql
